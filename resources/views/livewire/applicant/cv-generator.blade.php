<div class="pb-36 pt-16" style="background: #f0f4f9; min-height: 100vh;">
    <div class="max-w-2xl mx-auto px-4 pt-5">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-2xl font-extrabold text-black tracking-tight">Generator CV ATS</h1>
                <p class="text-black text-xs mt-1">Buat resume terstruktur standar industri yang lolos filter sistem ATS.</p>
            </div>
            <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-md border border-black text-black bg-white">
        ...
        @if(!$isPaid)
            ...
            <div class="bg-white rounded-3xl p-6 sm:p-8 text-center border border-slate-200 shadow-sm mb-6">
                <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4 bg-blue-50 text-black">
                    <i class='bx bx-file'></i>
                </div>
                <h2 class="text-xl font-bold text-black mb-1">Aktivasi Generator CV ATS Profesional</h2>
                <p class="text-black text-xs mb-6 max-w-sm mx-auto">Format terstruktur ramah scanner, langsung dibuat otomatis dari data profil Anda. Sekali bayar, aktif selamanya.</p>
                ...
                <div class="p-5 rounded-2xl mb-4 text-left max-w-md mx-auto border border-slate-200 bg-slate-50 relative">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-black">CV ATS Standalone</span>
                        <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border border-black text-black bg-white">Hemat 60%</span>
                    </div>
                    <div class="flex items-baseline justify-between mb-3">
                        <div>
                            <span class="text-xs text-black line-through block">Rp39.000</span>
                ...
                <div class="p-5 rounded-2xl text-left max-w-md mx-auto border border-black bg-white relative shadow-sm">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-xs font-bold text-black flex items-center gap-1">
                            <i class='bx bx-check-shield text-black'></i> Paket Komplit Siap Kerja
                        </span>
                        <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border border-black text-black bg-white">Rekomendasi</span>
                    </div>
                    <p class="text-xs text-black mb-3">Dapatkan <strong>1x CV ATS Resmi + 5 Kuota Lamaran Prioritas</strong> sekaligus!</p>
                    <div class="flex items-baseline justify-between">
                        <div>
                            <span class="text-xs text-black line-through block">Rp65.000</span>
                            <span class="text-2xl font-black text-black">Rp24.999</span>
                        </div>
                        <button wire:click="buyBundle" wire:loading.attr="disabled"
                                class="px-5 py-2.5 bg-[#5680d8] hover:bg-[#466ec2] active:scale-95 text-white font-bold text-xs rounded-xl shadow transition-all flex items-center gap-1.5">
        ...
        @else
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 rounded-xl bg-white text-black flex items-center justify-center text-xl font-black shrink-0 border border-black">
                        96%
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            <h4 class="font-bold text-black text-sm">Skor Kesesuaian ATS Tinggi</h4>
                            <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border border-black text-black bg-white">Siap Lolos HRD</span>
                        </div>
                        <p class="text-xs text-black mt-0.5">Format terstruktur, tanpa tabel tersembunyi, margin standar, dan ramah pembaca mesin.</p>
                    </div>
                </div>
            ...
            {{-- The CV Template --}}
            <div id="cv-document" class="bg-white p-8 sm:p-12 shadow-md border border-slate-200 text-black" style="font-family: 'Plus Jakarta Sans', sans-serif; max-width: 21cm; margin: 0 auto; min-height: 29.7cm;">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex justify-between items-baseline text-sm">
                        ...
                        <span class="text-black">Lulus</span>
                    </div>
                </div>
                    ...
                    <div class="mb-6">
                        ...
                        @foreach($profile->work_experiences as $exp)
                            <div class="mb-4">
                                ...
                                <div class="text-sm font-semibold text-black mb-1">{{ $exp['company_name'] ?? '-' }}</div>
                                <p class="text-sm text-black leading-relaxed text-justify">{{ $exp['description'] ?? '' }}</p>
                            </div>
                        @endforeach
                    </div>
                @endif
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
        <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-100 relative animate-scaleUp">
            ...
            <div class="text-center mb-5">
                ...
                <span class="text-[11px] font-bold uppercase tracking-wider text-black">Aktivasi Fitur CV ATS</span>
            ...
            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-5">
                ...
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-black">Total Tagihan:</span>
            ...
            <div class="space-y-2.5">
                ...
                <button wire:click="closeModal"
                        class="w-full py-2.5 text-black hover:text-black text-xs font-medium">
                    Batal
                </button>
            </div>

            <p class="text-[10px] text-center text-black mt-4">
