    ...
    (function(){
        ...
        window.njobOpenSheet=function(jobId){
            ...
            const html='<div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;">'
                ...
                +(!IS_AUTH
                    ? '<button onclick="window.Livewire.dispatch(\'open-quick-auth-modal\', {jobId: '+j.id+', title: \'Masuk / Buat Akun untuk Melamar\', subtitle: \'Lamar lowongan '+j.position.replace(/'/g, "\\'")+' di '+j.company.replace(/'/g, "\\'")+'\'})" style="flex:2;padding:14px;font-size:13px;font-weight:800;color:white;background:'+(j.hasWa?'#25D366':'#5680d8')+';border:none;border-radius:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 20px '+(j.hasWa?'rgba(37,211,102,.4)':'rgba(86,128,216,.4)')+';"><i class=\'bx '+(j.hasWa?'bxl-whatsapp':'bx-envelope')+'\'></i> '+(j.hasWa?'Lamar via WhatsApp':'Lamar via Email')+'</button>'
                    : (CRED>0
